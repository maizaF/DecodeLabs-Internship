...
const plants = [
  { 
    id: 1, name: "Golden Pothos", category: "foliage", neighborhood: "Riverside", 
    distance: 1.2, desc: "Trailing vine, low light okay. Cutting has 4 nodes.", 
    color: "#3F5D3A", icon: "eco", image: null, 
    care: "Low light, water when dry", size: "Medium", posted: Date.now() - 100000
  },
  { 
    id: 2, name: "Echeveria Pup", category: "succulent", neighborhood: "Maple Hill", 
    distance: 2.8, desc: "Rosette offset, ready to root. Needs bright light.", 
    color: "#8FAE83", icon: "spa", image: null,
    care: "Bright light, water sparingly", size: "Small", posted: Date.now() - 200000
  },
  { 
    id: 3, name: "Thai Basil", category: "herb", neighborhood: "Riverside", 
    distance: 0.6, desc: "Rooted cutting, very fragrant. Great for pesto.", 
    color: "#B5563C", icon: "grass", image: null,
    care: "Full sun, keep moist", size: "Small", posted: Date.now() - 300000
  },
  { 
    id: 4, name: "Marigold Seedlings", category: "flowering", neighborhood: "Oak Park", 
    distance: 3.5, desc: "Six seedlings, 2 weeks old. Full sun lovers.", 
    color: "#B7C9A8", icon: "local_florist", image: null,
    care: "Full sun, well-draining soil", size: "Seedling", posted: Date.now() - 400000
  },
  { 
    id: 5, name: "Spider Plant Baby", category: "foliage", neighborhood: "Maple Hill", 
    distance: 1.9, desc: "Well-established pup with roots already showing.", 
    color: "#3F5D3A", icon: "eco", image: null,
    care: "Medium light, tolerate neglect", size: "Small", posted: Date.now() - 500000
  },
  { 
    id: 6, name: "Jade Plant Cutting", category: "succulent", neighborhood: "Oak Park", 
    distance: 4.1, desc: "Two sturdy stems, calloused and ready to pot.", 
    color: "#8FAE83", icon: "spa", image: null,
    care: "Bright light, let soil dry", size: "Small", posted: Date.now() - 600000
  },
  { 
    id: 7, name: "Mint Runner", category: "herb", neighborhood: "Riverside", 
    distance: 0.9, desc: "Fast grower — comes with a warning about containers.", 
    color: "#B5563C", icon: "grass", image: null,
    care: "Part shade, keep moist", size: "Medium", posted: Date.now() - 700000
  },
  { 
    id: 8, name: "Nasturtium Starts", category: "flowering", neighborhood: "Willow Creek", 
    distance: 5.2, desc: "Edible flowers, peppery leaves, easy to grow.", 
    color: "#B7C9A8", icon: "local_florist", image: null,
    care: "Full sun to part shade", size: "Small", posted: Date.now() - 800000
  },
];
const state = {
  category: "all",
    ...
    grid.innerHTML = sorted.map(p => {
      ...
      return `
        <article class="plant-card" data-id="${p.id}">
          <div class="card-photo" style="background:${p.color}">
            ${imageHtml}
          </div>
          <div class="card-body">
...
grid.addEventListener("click", (e) => {
  ...
  if (favBtn) {
    ...
    showToast(
      state.favorites.has(id) ? `❤️ Saved ${plant.name} to favorites` : `Removed ${plant.name} from favorites`,
      "info"
    );
    return;
  }
  ...
  if (isValid) {
    ...
    setTimeout(() => {
      const newPlant = {
        ...
        color: ["#3F5D3A", "#8FAE83", "#B5563C", "#B7C9A8"][Math.floor(Math.random() * 4)],
        icon: ["eco", "spa", "grass", "local_florist"][Math.floor(Math.random() * 4)],
      ...
      showToast(`🌱 ${newPlant.name} listed successfully!`, "success");
...
function openDetailModal(plantId) {
  ...
  detailModal.innerHTML = `
    <div class="modal" role="dialog" aria-modal="true" aria-labelledby="detailTitle">
      ...
      <div style="margin: 1rem 0; padding: 0.5rem; background: ${plant.color}20; border-radius: 8px; display: flex; align-items: center; gap: 0.5rem;">
        <span class="material-symbols-outlined" style="color:${plant.color}">local_florist</span>
